...
func RunTest(t *testing.T, storage zanzigo.Storage, expectations Expectations) {
	...
	t.Run("list", func(t *testing.T) {
		...
		tuples, newCur, err := storage.List(ctx, zanzigo.Tuple{ObjectType: "doc", ObjectID: "mydoc"}, zanzigo.Pagination{Cursor: storage.CursorStart(), Limit: 10})
		require.NoError(t, err)
		require.Equal(t, 2, len(tuples))
		require.NotEqual(t, storage.CursorStart(), newCur)
